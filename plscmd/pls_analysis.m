...
function result = pls_analysis(datamat_lst, num_subj_lst, k, opt)
   ...
   if nargin == 4 & ~isempty(opt)
      ...
      if isfield(opt,'cormode')
         ...
         if isempty(cormode) | ~ismember(cormode,[0 2 4 6 8])
            error('Field "cormode" should be 0, 2, 4, 6 or 8');
         end
         
         if cormode == 8 & ~isfield(opt, 'stacked_behavdata')
             error('Field "stacked_behavdata" must be defined for Spearman correlation')
         end 
